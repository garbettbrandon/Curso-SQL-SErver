...
-- 3 ----------------------------------------------------
INSERT INTO
  [dbo].[orders] ([customer_id], [order_date])
VALUES
  (100, '2024-01-05')
  /* Una vez que insertes la nueva fila en la tabla orders, devuelve el ID del cliente, el nombre del
   ...
   valor no est� disponible, puedes dejarlo en blanco (NULL). Ordena la salida por customer_id
...
FROM
  ...
  LEFT JOIN orders o ON c.customer_id = o.customer_id
  AND dr.order_date = o.order_date
WHERE
  o.order_id IS NULL -- Solo queremos los pares sin pedidos
ORDER BY
  c.customer_id,
  dr.order_date;